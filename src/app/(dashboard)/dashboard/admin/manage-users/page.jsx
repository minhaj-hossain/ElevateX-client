...
export default function ManageUsersPage() {
  ...
  // Load standard structural database profile arrays
  const fetchUsersData = async () => {
    try {
      ...
      console.error(err);
      toast.error(
        "Error establishing stable network bridge synchronization data pipelines.",
      );
    } finally {
      setLoading(false);
    }
  ...
  // Modify User Status (Block/Unblock)
  const handleToggleBlock = async (userId, currentStatus) => {
    try {
      ...
      if (data.success) {
        ...
        // Instant state mutation swap to preserve active DOM focus
        setUsers((prev) =>
          prev.map((u) =>
            u._id === userId ? { ...u, status: data.newStatus } : u,
          ),
        );
        // Refresh analytic balance blocks
        setMetrics((prev) => ({
          ...prev,
          flaggedAccounts:
        ...
        toast.error(data.error || "Execution fault encountered.");
      }
    } catch (err) {
      toast.error("Operation mutation channel interrupted.");
    }
  };

  // Escalate to System Administrator role
  const handleMakeAdmin = async (userId) => {
    try {
      ...
      if (data.success) {
        toast.success("Security permissions updated: User is now Admin.");
        setUsers((prev) =>
          prev.map((u) => (u._id === userId ? { ...u, role: "Admin" } : u)),
        );
      } else {
        toast.error(data.error || "Escalation structural anomaly.");
      }
    } catch (err) {
      toast.error("Access modification matrix error.");
    }
  };
  ...
  return (
    <div className="bg-[#0a0a0c] text-white min-h-screen p-6 md:p-10 font-sans selection:bg-[#c4e42a] selection:text-black">
      <div className="max-w-7xl mx-auto space-y-8">
        {/* Top Control Title Heading Container Section */}
        ...
        {/* Analytic Performance Statistics Display Summary Rows (from image_f46e5b.png) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Box 1: Total Users */}
          <div className="bg-[#121214] border border-zinc-900/60 rounded-xl p-5 relative">
        ...
        {/* Core Administrative Accounts Matrix Roster Data Grid Table Panel */}
        <div className="bg-[#121214] border border-zinc-900 rounded-xl p-6 overflow-hidden">
          {loading ? (
            ...
            <div className="text-zinc-600 text-xs font-black uppercase tracking-widest text-center py-20">
              No registered user platform profiles detected.
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-zinc-900/40 text-xs font-bold">
                  {users.map((user) => {
                    ...
                    return (
                      <tr
                        key={user._id}
                        className={`group transition-colors ${isBlocked ? "opacity-40 hover:opacity-60 bg-red-950/5" : "hover:bg-zinc-900/20"}`}
                      >
                        {/* Name Column Context Segment Block */}
                        <td className="py-4 pr-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <span
                                className={`block uppercase tracking-wide text-zinc-100 ${isBlocked && "line-through text-zinc-500"}`}
                              >
                              ...
                              <span className="block text-[10px] text-zinc-500 font-medium tracking-tight mt-0.5">
                                {isBlocked
                                  ...
                                  : `Joined ${user.joinDate || "Recent Pipeline"}`}
                              </span>
                            </div>
                          </div>
                        </td>

                        {/* Email Column Context Segment */}
                        <td className="py-4 pr-4 text-zinc-400 font-medium selection:bg-zinc-800">
                          {user.email}
                        </td>

                        {/* Role Column pill badges layout */}
                        <td className="py-4 pr-4">
                          <span
                        ...
                        {/* Status dynamic pill row rendering segment */}
                        <td className="py-4 pr-4">
                          <div className="flex items-center gap-1.5">
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${isBlocked ? "bg-red-500 shadow-[0_0_6px_#ef4444]" : "bg-[#c4e42a] shadow-[0_0_6px_#c4e42a]"}`}
                            />
                            <span
                              className={`text-[11px] uppercase tracking-wide ${isBlocked ? "text-red-400" : "text-zinc-200"}`}
                            >
                              {user.status || "Active"}
                            </span>
                          </div>
                        ...
                        {/* Action buttons rendering conditionally wrapper block column layout */}
                        <td className="py-4 text-right space-x-2 whitespace-nowrap">
                          {/* Render Role Expansion Elevation path only for standard tiers accounts */}
                          {!isAdmin && (
                            <button
                          ...
                          {/* Block / Unblock control path toggle dynamic buttons structural interface link blocks */}
                          <button
                            onClick={() =>
